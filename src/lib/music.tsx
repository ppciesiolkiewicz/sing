...
export const NoteModule = {
  ...
  areNotesEqual: (n1: NoteType, n2: NoteType): boolean =>
    n1.chroma === n2.chroma && (!n1.oct || n1.oct === n2.oct),
  areNotesSameNote: (n1: NoteType, n2: NoteType): boolean =>
    n1.chroma === n2.chroma,
  getGuitarNotes: (tuning: string[], fretCount: number): NoteType[][] => {
    return tuning.map((rootNote) => {
      return times(fretCount - 1, (fretNo) => {
        const noteSymbol = transpose(
          rootNote,
          TonalInterval.fromSemitones(fretNo)
        );
...
export const ChordModule = {
      ...
      return {
        ...
        notes: octave
          ? chord.intervals.map((interval) =>
              NoteModule.transpose(`${chord.notes[0]}${octave}`, interval)
            )
          : chord.notes,
        suffix,
      };
  ...
  getAllChords: (
    ...
    return notes.map((n) => chordSymbols.map((cs) => `${n.name}${cs}`)).flat();
  },
  getAllRelevantChords: (
    lowestNoteName: string,
    hightestNoteName: string
  ): string[] => {
    const chordSymbols = [
...
export const ChordProgressionModule = {
  create: (
    ...
    return TonalProgression.fromRomanNumerals(
      keyTonic,
      progressionNumerals
    ).map((chordName) => {
...
export const ScaleModule = {
  ...
  getScaleNotes(
    ...
    const chromaticNotes = NoteModule.getNoteRange(
      lowestNoteName,
      highestNoteName
    );
  ...
  getScaleChords(scale: ScaleType, octaves = [2]) {
    ...
    const indexOfTonicChord = scale.keyChords.triads.indexOf(
      (c) => c === tonic
    );
  ...
  splitIntoRangesByTonic(keyTonic: string, notes: NoteType[]) {
    const splitIndices = [
      0,
      ...notes.reduce(function (acc, note, i) {
        // TODO: e[0]
        if (
          keyTonic.length == 2 &&
          `${note.name[0]}${note.name[1]}` === keyTonic
        ) {
          acc.push(i);
    ...
    const splittedNotes = splitIndices.map((splitIdx, i) => {
