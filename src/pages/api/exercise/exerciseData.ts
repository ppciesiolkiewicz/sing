...
export const getScaleExercises = (
  ...
  const SCALE_EXERCISES = NoteModule.names()
    .map((keyTonic) =>
      ScaleModule.relevantNames().map((keyType) => {
        ...
        const scaleNotesParts = ScaleModule.splitIntoRangesByTonic(
          keyTonic,
          scaleNotes
        );
...
export const getIntervalExercises = (
  lowestNoteName: string,
  highestNoteName: string
) => {
  const INTERVAL_ERXERCISES = [
    ...
    description: `Intervals: ${e.intervalNames.join(
      "-"
    )}. Exercise range ${lowestNoteName} — ${highestNoteName}`,
